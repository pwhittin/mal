...
(defn mal-sequence->string [start-token end-token mal-sequence print-readably]
  (str start-token (->> mal-sequence (map #(print-str % print-readably)) (interpose " ") (apply str)) end-token))
...
(defn print-str
  ...
  ([[mal-type mal-value] print-readably]
   (case mal-type
     ...
     :mal-map (mal-map->string mal-value print-readably)
     :mal-nil "nil"
